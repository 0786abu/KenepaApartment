...
import { Delete_Property } from '../redux/action/propertyAction';
...
const Admin_All_Properties = () => {
    ...
    const {p_loading,allproperties,deleteloading} = useSelector((state)=>state.Admin);
  ...
  {p_loading ? (<Loader/>):allproperties?.length===0 ? (<div className=' flex justify-center items-center h-[80vh]'>
    ...
    <tbody className="whitespace-nowrap">
      {allproperties?.map((item,index)=>{
        ...
        <td className="p-4">
          <Link to={`/admin-dashboard-updateproperty/${item._id}`}>
          ...
          <button onClick={()=>handleDelete(item._id)} disabled={deleteloading} title="Delete">
            {deleteloading && itemid === item._id ? (
