...
import { Delete_Review, Fetch_Reviews, Update_Review } from '../redux/action/reviewAction';
...
const All_Reviews = () => {
    ...
    const {reviewloading,reviews,reviewdelloading,revupdateloading} = useSelector((state)=>state.Review);
    ...
    const handleupdate = (e)=>{
        e.preventDefault()
        dispatch(Update_Review(itemid,contactdata,handleClose))
    }
    useEffect(()=>{
        if(contact){
            setContactdata({
           ...
           <form className="formm border p-2 md:p-4 w-full shadow-md" onSubmit={handleupdate}>
                                 <div className="wrapper">
                                     <h2 className=" text-3xl font-sans font-[600] mb-4">Get A Quote</h2>
                                         ...
                                         <div className="input w-full relative my-2">
                                             <input type="text" placeholder="Enter Position" value={contactdata.position} onChange={(e)=>setContactdata({...contactdata,position:e.target.value})} className=" border-2 border-gray-100 pl-4 outline-none h-14 w-full" />
                                         </div>
             ...
             <tbody className="whitespace-nowrap">
               {reviews?.map((item,index)=>{
                 ...
                 <td className="p-4 text-sm">
                  <img src={`${import.meta.env.VITE_BASE_URL}${item.profile}`} className=' w-12 h-12 rounded-full object-cover' alt="" />
                 </td>
                 <td className="p-4 text-sm">
                 {item.name}
                 </td>
                 ...
                 <td className="p-4">
                  ...
                  {(reviewdelloading && item._id === itemid) ? (<Spinnerr/>):(
